...
public class UserService {
	...
	public List<User> selectALL() throws Exception {
		...
		// 1-2. 데이터 가공 (할 거 없으니 패스)
		// 2. DAO 메서도 호출(SELECT) 후 결과반환(List<User>) 받기
		List<User> userList = dao.selectAll(conn);
		
		// INSERT나 UPDATE 등 DML이 아니기에 COMMIT, ROLLBACK 안함
		// 3. Connection 반환
		JDBCTemplate.close(conn);
	...
	public int idCheck(String userId) throws Exception {
		...
		//SELECT라 commit/rollback 필요없으니 닫기
		JDBCTemplate.close(conn);
	...
	public int multiInsertUser(List<User> userList) throws Exception {
		...
		int count = 0; // 삽입 성공한 행의 개수 누적 count
