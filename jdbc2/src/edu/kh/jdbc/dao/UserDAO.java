...
public class UserDAO {
	...
	// 메서드
	/**
	 * 전달받은 Connection을 이용해서 DB에 접근하여 전달받은 아이디(input)와 일치하는 User 정보를 DB 조회하기
	 * 
	...
	public int insertUser(Connection conn, User user) throws Exception {
		...
		// SQL 수행 중 발생하는 예외를
		// catch로 처리하지 않고, throws를 이용해서
		// UserView catch문으로 던져 처리
		// -> 해당 메소드엔 catch 문 필요없다!
		...
		try {
			...
			close(pstmt);
			// JDBCTemplate.close(pstmt); > 위에 close와 같음
	...
	public List<User> selectAll(Connection conn) throws Exception {
		...
		try {
			...
			// 3. PreparedStatement 생성 (위치가 없어도 사용할 수 있다)
			pstmt = conn.prepareStatement(sql);
			...
			while (rs.next()) { // 행이 있다
				...
				// -> SELECT 문에서 TO_CHAR()를 이용하여 문자열로 변환해
				// 조회했기 때문에
	...
	public User selectUser(Connection conn, int input) throws Exception {
		...
		try {
			...
			if (rs.next()) {
				...
				// 위에 User을 선언해두었기 때문에 밑에 여기서 user 대입만 하면 된다
				// User user = ~ 이렇게 적는 거는 User 선언이 되어 있지 않을 시 하는 것
				// User을 두번 적을 수 없다! >> 쳇지피티로 다시 공부하는 게 좋아보임
			}
	...
	public int deleteUser(Connection conn, int input) throws Exception {
		...
		try {
			...
			result = pstmt.executeUpdate(); // result에 값을 담겠다.
